...
fig, ax = plt.subplots(nrows=int(spectra.shape[0]) + 4, ncols=1, figsize=(22, 17), sharex=True, sharey=True)
# plt.tight_layout()
...
print(index)
real_spectra = np.sum(spectra ** 2, axis=2)
real_spectra = real_spectra / (np.max(real_spectra))
max_y = 1.1*np.max(spectra)
min_y = 1.1*np.min(spectra)
