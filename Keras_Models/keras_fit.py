...
def data_generator(transfer='TF_train_wave_unwrapped_eggs.hdf5', batch_size=64, cut_bot=.8, cut_top=1., reps=20):
    ...
    for batch in np.arange(start=0, stop=random_shuffled_index.shape[0], step=batch_size):
        ...
        magphase = magnitude * phase/phase_scale_factor
        yield (Spectra16[np.sort(random_shuffled_index[batch: batch + batch_size]), ...],
               {'magnitude': magnitude,
                'phase': magphase})


train_data = data_generator(transfer='../Data/unwrapped/Eggs/TF_train_wave_unwrapped_eggs.hdf5',
                            batch_size=64,
                            cut_bot=.0,
                            cut_top=.8)
test_data = data_generator(transfer='../Data/unwrapped/Eggs/TF_train_wave_unwrapped_eggs.hdf5',
                           batch_size=64,
                           cut_bot=.8,
                           cut_top=1.0)
...
tf.keras.models.save_model(model=keras_model, filepath=filename)
#del keras_model
