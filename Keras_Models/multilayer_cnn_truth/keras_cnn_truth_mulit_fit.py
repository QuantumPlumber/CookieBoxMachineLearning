...
def data_generator(transfer='TF_train_wave_unwrapped_eggs.hdf5', batch_size=64, cut_bot=.8, cut_top=1., reps=1):
    ...
    while True:
        ...
        random_shuffled_index = np.sort(np.unique(np.tile(random_shuffled_index, reps=reps)[0:sample_size]))

        local_time = Time_pulse[random_shuffled_index, ...]
        local_nonlinearphase = Nonlinearphase_pulse[random_shuffled_index, :]
        local_spectra = Spectra[random_shuffled_index, ...]

        sort_index = np.arange(start=0, stop=random_shuffled_index.shape[0])

        sample_num = 0
        while True:
            num_spike = np.random.randint(low=1, high=5, size=batch_size)
            magnitude_list = []
            nonlinear_phase_list = []
            spect_pb_list = []
            for num in num_spike:
                add_index = np.sort(np.random.choice(sort_index, num, replace=False))
                magnitude_list.append(mag_scale_factor * np.sum(local_time[add_index, :], axis=0, keepdims=True))
                nonlinear_phase_list.append(np.sum(local_nonlinearphase[add_index, :], axis=0, keepdims=True))
                spect = np.sum(local_spectra[add_index, ...], axis=0, keepdims=True)
                spect_pb_list.append((spect[:, :, :, 0] ** 2 + spect[:, :, :, 1] ** 2) * 1e9)

            yield (
                np.concatenate(spect_pb_list, axis=0),
                {'magnitude': np.concatenate(magnitude_list, axis=0),
                 'phase': np.concatenate(nonlinear_phase_list, axis=0)})

            if sample_num == sample_batch_break:
                break
